...
async function preBind({
	...
	if (userMatched.length > 0) {
		...
		console.log("preBind中匹配到的用户:", userMatched);
		throw {
...
async function preUnBind({
	...
	const {
		data: users
	} = await userCollection.where(dbCmd.and({
			_id: uid
		},
		dbCmd.or(condition)
	)).get()
	...
	const [user] = users
	const otherAccounts = batchFindObjctValue(user, Object.keys(USER_IDENTIFIER).filter(key => !notUnBind.includes(
		key) && !currentUnBindAccount.includes(key)))
